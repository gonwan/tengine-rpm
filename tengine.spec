...
Obsoletes:         tengine-mod-http-geoip <= 1:1.16
%else
...
%if %{with geoip}
Requires:          tengine-mod-http-geoip = %{epoch}:%{version}-%{release}
%endif
Requires:          tengine-mod-http-image-filter = %{epoch}:%{version}-%{release}
Requires:          tengine-mod-http-perl = %{epoch}:%{version}-%{release}
Requires:          tengine-mod-http-xslt-filter = %{epoch}:%{version}-%{release}
Requires:          tengine-mod-mail = %{epoch}:%{version}-%{release}
Requires:          tengine-mod-stream = %{epoch}:%{version}-%{release}
...
The tengine-filesystem package contains the basic directory layout
...
ln -s tengine-logo.png %{buildroot}%{_datadir}/nginx/html/poweredby.png
...
install -p -D -m 0644 %{_builddir}/tengine-%{version}/objs/nginx.8 \
...
%files
%license LICENSE
%doc CHANGES README.markdown README.dynamic
%if 0%{?rhel} == 7
